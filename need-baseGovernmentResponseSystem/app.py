...
def api_submit_request():
    """Submit a new relief request with improved error handling"""
    try:
        data = request.json
...
def api_create_staff():
    ...
    try:
        ...
        if not department_name:
            cur.close()
            conn.close()
            return jsonify({'success': False, 'error': 'Department is required', 'message': 'Department is required'}), 400
        ...
        if not department_id:
            cur.close()
            conn.close()
            return jsonify({'success': False, 'error': f'Invalid department: {department_name}', 'message': f'Department "{department_name}" not found in database'}), 400
        ...
        elif department_name in ['Health and Medical Services', 'Mental Health', 'Dental Services', 'Immunization', 'Laboratory and Diagnostics']:
...
def api_update_staff(staff_id):
    ...
    try:
        ...
        # Get department name
        cur.execute("SELECT d.department_name FROM departments d JOIN staff s ON s.department_id = d.department_id WHERE s.staff_id = %s", (staff_id,))
    ...
    try:
        ...
        query = """
            SELECT p.professional_id, p.full_name, p.email, p.phone, p.profession_type,
                   ...
                   p.status, p.max_appointments_per_day, p.total_appointments, p.rating
        ...
        query += " ORDER BY p.rating DESC, p.full_name"
