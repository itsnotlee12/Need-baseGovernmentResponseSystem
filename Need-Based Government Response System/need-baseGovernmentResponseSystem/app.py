...
def save_request_to_db(request_data):
    ...
    try:
        ...
        # Extract location address (handle both string and dict)
...
def generate_staff_official_id(conn):
    ...
    try:
        cur = conn.cursor()
        cur.execute("SELECT MAX(CAST(SUBSTRING(employee_id FROM 'GOV-([0-9]+)') AS INTEGER)) FROM staff WHERE employee_id LIKE 'GOV-%'")
        result = cur.fetchone()
...
def get_allowed_need_types_for_role(role, department):
    ...
    """
    # Department-based mapping for Officers and other roles
    # Officers are filtered by their specific department within health services
    department_mapping = {
        ...
        'Health and Medical Services': ['medical', 'dental-services', 'immunization', 'laboratory', 'mental-health', 'sanitary-inspection'],  # All health services
        'Sanitary Inspection': ['sanitary-inspection'],  # Officers in Sanitary Inspection department
        'Medical': ['medical'],  # Officers in Medical department
        'Dental Services': ['dental-services'],  # Officers in Dental Services department
        'Immunization': ['immunization'],  # Officers in Immunization department
        'Laboratory and Diagnostics': ['laboratory'],  # Officers in Laboratory department
        'Mental Health': ['mental-health']  # Officers in Mental Health department
    }
    ...
    role_mapping = {
        ...
        'officer': None  # Officers must be filtered by their specific department
    }
                    ...
                    try:
                        ...
                        if prof_record and prof_record[2]:
                            if verify_password(password, prof_record[2]):
                                ...
                                print(f"DEBUG: Professional login successful - Name: {name}, Specialization: {prof_record[1]}")
                    ...
                    try:
                        ...
                        if staff_record and staff_record[3]:
                            if verify_password(password, staff_record[3]):
                                ...
                                print(f"DEBUG: Staff login successful - Name: {name}, Dept: {department}, Role: {user_position}")
...
def api_get_requests():
    ...
    if session.get('user_role') == 'government':
        ...
        print(f"DEBUG: Filtered to {len(filtered_requests)} requests")
...
def api_submit_request():
    ...
    if isinstance(location_data, dict):
        ...
        location_address = str(location_data)
    ...
    queue_position = len(pending_requests)
    ...
    for req in requests_db:
        if req['id'] == request_id:
            ...
            if new_status == 'completed':
                ...
                # Check if this is a mental health request that needs notification
                    ...
                    try:
                        ...
                        if conn:
                            ...
                            if appt_data:
                                ...
                                notification_title = "Mental Health Appointment - Waiting for Confirmation"
                                notification_message = f"""
Your mental health appointment has been scheduled.

Status: Waiting for professional confirmation

A mental health professional will review and confirm your appointment. You will receive another notification with the doctor's name, contact details, and appointment confirmation.
...
"""
                                ...
                                """, (req['email'], notification_title, notification_message, request_id, appointment_id))
...
def api_create_staff():
    ...
    try:
        ...
        """, (
            ...
            password_hash,
            data.get('phone'),
            official_id,  # Auto-generated official ID
            data.get('department'),
    ...
    try:
        ...
        cur.execute("""
            SELECT a.citizen_email, a.citizen_name, a.appointment_date, a.appointment_time,
                   p.full_name, p.phone, a.request_id
            FROM appointments a
        ...
        citizen_email, citizen_name, appt_date, appt_time, prof_name, prof_phone, request_id = appointment
        ...
        # Create notification for citizen
        notification_title = "Mental Health Appointment Confirmed"
        notification_message = f"""
Your mental health appointment has been confirmed!
