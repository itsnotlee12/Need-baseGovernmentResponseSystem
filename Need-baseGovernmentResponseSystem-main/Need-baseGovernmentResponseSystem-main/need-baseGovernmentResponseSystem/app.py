...
# ============================================
# SANITARY INSPECTION SCHEDULING
# ============================================

@app.route('/api/staff/schedule_sanitary_inspection', methods=['POST'])
def schedule_sanitary_inspection():
    """Staff schedules a sanitary inspection - notifies citizen only"""
    if 'user_email' not in session:
        return jsonify({'success': False, 'error': 'Unauthorized - Please login'}), 401
    ...
    inspection_type = data.get('inspection_type', 'on-site-inspection')
    notes = data.get('notes', '')
    ...
    try:
        ...
        # Get request details
        cur.execute("""
            SELECT citizen_name, email, phone, need_type, location_address
            FROM requests
            WHERE request_id = %s
        """, (request_id,))
        ...
        citizen_name, citizen_email, citizen_phone, need_type, location_address = request_data

        if need_type != 'sanitary-inspection':
            return jsonify({'success': False, 'error': 'This request is not for sanitary inspection'}), 400

        # Update request referral status to 'scheduling' (valid value)
        cur.execute("""
            UPDATE requests
            SET referral_status = 'scheduling', 
                status = 'in-progress',
        ...
        notification_title = "Sanitary Inspection Scheduled"
        notification_message = f"""
Dear {citizen_name},

Your sanitary inspection has been scheduled!

📅 Scheduled Date: {inspection_date}
📐 Scheduled Time: {inspection_time}
📍 Location: {location_address}
 📋 Inspection Type: {inspection_type.replace('-', ' ').title()}

{f'Additional Information: {notes}' if notes else ''}

What to expect:
 A sanitary inspector will visit the location at the scheduled time.
 Please ensure the area is accessible.
 The inspection typically takes 30-60 minutes.

Important Notes:
Please be present at the location during the scheduled time.
 If you need to reschedule, please contact us immediately.
 Have any relevant documents or information ready.

Request ID: {request_id}

If you have any questions, please don't hesitate to contact the City Health Office.

Best regards,
City Health Office - Sanitary Inspection Team
"""
        ...
        """, (citizen_email, 'citizen', 'appointment_scheduled', notification_title,
              notification_message, request_id, False))

        # Log the action
        cur.execute("""
            INSERT INTO audit_logs (action_type, user_email, user_role, entity_type, entity_id, details)
            VALUES ('CREATE', %s, 'government', 'INSPECTION_SCHEDULE', %s, %s)
        """, (session['user_email'], request_id,
              f'Sanitary inspection scheduled for {inspection_date} at {inspection_time}'))
        ...
        return jsonify({
            'success': True,
            'message': 'Sanitary inspection scheduled successfully',
            'request_id': request_id
        })

    except Exception as e:
        print(f"Error scheduling sanitary inspection: {e}")
        import traceback
        traceback.print_exc()
        if conn:
            conn.rollback()
...
def update_request_status(request_id):
    ...
    valid_statuses = ['pending', 'reviewing', 'approved', 'scheduling', 'scheduled',
    ...
    try:
        ...
        # Get request details
        cur.execute("""
            SELECT citizen_name, email, need_type
            FROM requests
            WHERE request_id = %s
        """, (request_id,))
        ...
        citizen_name, citizen_email, need_type = request_data
        ...
        # Send notification to citizen if status is completed
        if new_status == 'completed':
            service_name = need_type.replace('-', ' ').title()
            notification_title = f"Request Completed - {service_name}"
            notification_message = f"""
Dear {citizen_name},

Your {service_name} request has been completed!

Request ID: {request_id}
Status: Completed â

Thank you for using our City Health Services. We hope we were able to address your needs effectively.

If you have any follow-up questions or concerns, please don't hesitate to contact us or submit a new request.

Best regards,
City Health Office Team
"""
            ...
            """, (citizen_email, 'citizen', 'request_update', notification_title,
                  notification_message, request_id, False))

        # Log the action
        cur.execute("""
            INSERT INTO audit_logs (action_type, user_email, user_role, entity_type, entity_id, details)
            VALUES ('STATUS_CHANGE', %s, %s, 'REQUEST', %s, %s)
        """, (session['user_email'], session.get('user_role', 'government'), request_id,
